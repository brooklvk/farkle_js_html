...
// THIS COPY IS THE SCRIPT BEFORE ATTEMPTING TO ADD KEEP-TRACK-OF-INDEXES FUNCTIONALITY. 
// THIS COPY WORKS, HAS ROLL-DICE, ROLL-AGAIN, AND STOP-ROLLING BUTTONS 
// BUT IT DOESN'T ACTUALLY ADD UP THE SCORE AS YOU GO 
// AND THE USER CAN'T CHOOSE WHICH DICE TO KEEP / ROLL 
// also the scoring doesn't calculate 1s and 5s 
...
// Scoring ignores the score of individual 1s or 5s dice 
function calcRollScore() {
    
    const allDiceObject = createAllDice();

    // Variable would be diceArrayNumbers, but is dice for purpose of this function's if statements.
    ...
    return rollScore; // An object {result: score: }
}

let lastRolled = calcRollScore();
